...
import SelectData from "./dropdown-list-countries";
...
export default function FormAddress({ dataProps, isAdd }: IProps) {
  const {
    selectedCountry,
    selectedState,
    setSelectedCountry,
    ...
    handleSave,
    loadding
  } = useShippingHook(isAdd, dataProps);
  const active =
        ...
        <form className="grid grid-cols-1 max-w-[720px] gap-x-[14px] gap-y-4 sm:grid-cols-2">
          ...
          <div className="flex justify-end w-full sm:col-span-2">
            <button
              ...
              disabled={!active}
              className={`${
                active ? "bg-[#242424]" : "bg-[#D6D6D6]"
              } mt-10 ${loadding ? "" : "max-w-[164px]"} justify-center rounded-md px-5 py-4 text-sm font-bold text-white shadow-sm ring-1 ring-inset ring-gray-300 hover:opacity-95 sm:w-auto`}
            >
               {loadding ? (
                <svg
                  aria-hidden="true"
